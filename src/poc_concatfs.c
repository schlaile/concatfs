...
{
	struct concat_file * c = open_concat_file(0, path);
	off_t rv;
...
static int read_concat_file(int fd, void *buf, size_t count, off_t offset)
{
	...
	for (c = cf->chunks; c && offset > c->fsize; c = c->next) {
		offset -= c->fsize;
	}
...
static int concatfs_getattr(const char *path, struct stat *stbuf)
{
	...
	if (is_concatfs_file(path)) {
		stbuf->st_size = get_concat_file_size(path);
	} 
...
int main(int argc, char **argv)
{
	...
	memcpy(argv_ + 1, argv + 1, (argc - 2) * sizeof(char*));
